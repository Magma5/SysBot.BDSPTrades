    ...
    public class TradeModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task TradeAsync(int code, string content, bool isRandomCode) {
            ...
            try
            {
                ...
                var sig = Context.User.GetFavor();
                await AddTradeToQueueAsync(code, Context.User.Username, new T[] { pk }, sig, Context.User, isRandomCode).ConfigureAwait(false);
            }
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        private async Task TradeAsyncAttach(int code, RequestSignificance sig, SocketUser usr, bool isRandomCode)
        {
            ...
            await AddTradeToQueueAsync(code, usr.Username, attchList.ToArray(), sig, usr, isRandomCode).ConfigureAwait(false);
        }
        ...
        private async Task AddTradeToQueueAsync(int code, string trainerName, T[] pks, RequestSignificance sig, SocketUser usr, bool isRandomCode)
        {
            ...
            await QueueHelper<T>.AddToQueueAsync(Context, code, trainerName, sig, pks, PokeRoutineType.BDSPLinkTrade, PokeTradeType.Specific, usr, isRandomCode).ConfigureAwait(false);
        }
    }
}
